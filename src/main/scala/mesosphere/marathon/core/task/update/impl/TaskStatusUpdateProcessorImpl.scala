...
private[this] object Helper {
  ...
  def killWhenUnknownOrNotLaunched(status: MesosProtos.TaskStatus): Boolean = {
    !ignoreWhenUnknown.contains(status.getState)
  }

  def taskKnownOrNotStr(maybeTask: Option[Task]): String = if (maybeTask.isDefined) "known" else "unknown"
}
